...
export const useLayoutSaving = (
  ...
  const convertSlideToReact = async (slide: ProcessedSlide, presentationId: string, FontUrls: string[]) => {
    ...
    while (retryCount < maxRetries) {
      try {
        const response = await fetch("/api/v1/html-to-react/", {
          method: "POST",
          headers: {
  ...
  const saveLayout = useCallback(async (layoutName: string, description: string): Promise<string | null> => {
    ...
    try {
      ...
      await fetch("/api/v1/template-management/templates", {
        method: "POST",
      ...
      const saveResponse = await fetch(
        "/api/v1/template-management/save-templates",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  }, [slides, refetch, closeSaveModal, setSlides]);
